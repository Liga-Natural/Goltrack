...
export function BracketView({ matches, teams, sport }: { matches: Match[]; teams: Team[]; sport: string }) {
  ...
  if (orderedRounds.length === 0) {
    return <p className="text-ink3 text-sm">No knockout bracket yet.</p>;
  }
    ...
    <div className="flex gap-6 overflow-x-auto pb-4">
      {orderedRounds.map(([roundName, roundMatches]) => (
        <div key={roundName} className="flex flex-col justify-around gap-4 min-w-[220px]">
          <h4 className="text-xs uppercase tracking-wide text-ink3 text-center mb-1">{roundName}</h4>
          {roundMatches.map((m) => (
            <div key={m.id} className="card p-3">
              <div className={`flex items-center justify-between text-sm py-1 ${m.status === "FINAL" && (m.homeScore ?? 0) > (m.awayScore ?? 0) ? "font-semibold text-accent" : ""}`}>
              ...
              <div className={`flex items-center justify-between text-sm py-1 border-t border-black/5 ${m.status === "FINAL" && (m.awayScore ?? 0) > (m.homeScore ?? 0) ? "font-semibold text-accent" : ""}`}>
