    ...
    <div>
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-2">
          ...
          {matches.length === 0 && <p className="text-ink2 text-sm">Generate a schedule first.</p>}
          {matches.map((m) => {
            ...
            return (
              <div key={m.id} className="card p-4 flex flex-wrap items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs text-ink3">{m.round} · {m.field}</p>
        ...
        <div>
          ...
          <div className="card p-5 lg:sticky lg:top-6">
            ...
            <div className="space-y-1.5 mb-4">
              {referees.map((r) => (
                <div key={r.id} className="text-sm text-ink2 border-b border-black/5 pb-1">
                  {r.name} {r.contact && <span className="text-ink3">· {r.contact}</span>}
                </div>
              ))}
              {referees.length === 0 && <p className="text-xs text-ink3">No referees added yet.</p>}
            </div>
